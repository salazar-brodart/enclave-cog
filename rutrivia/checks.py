...
__all__ = ("rutrivia_stop_check",)

_ = Translator("rutrivia", __file__)


def rutrivia_stop_check():
    async def predicate(ctx: commands.GuildContext) -> bool:
        session = ctx.cog._get_rutrivia_session(ctx.channel)
        if session is None:
            raise commands.CheckFailure(_("There is no ongoing rutrivia session in this channel."))
