...
class RoleManagement(
    ...
    async def srole_buy(self, ctx: commands.GuildContext, *, role: discord.Role):
        ...
        try:
            ...
            if not eligible:
                return await ctx.send(
                    f"Не пытайся смухлевать! В следующий раз сообщу кому нужно."
                )

            if not cost:
                return await ctx.send(
                    "Не пытайся смухлевать! В следующий раз сообщу кому нужно."
                )
            ...
            try:
                ...
                return await ctx.send(
                    f"А ну вали отсюда! И не возвращайся, пока не соберешь {cost} {currency_name}."
                )
            else:
    ...
    async def sadd(self, ctx: commands.GuildContext, role: discord.Role):
        ...
        try:
            ...
            if not eligible:
                ...
                await ctx.send(
                    "Вообще-то это стоит денег."
                    "Сходи-ка ты до Блескотрона и раскошелься."
                )
            else:
                await self.update_roles_atomically(
                    who=ctx.author, give=[role], remove=remove
