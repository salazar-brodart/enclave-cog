...
class enclave(commands.Cog):
    ...
    async def это_тест(self, ctx: Context, user = None):
        ...
        await ctx.send(f"Mention - {ctx.author.mention}\nDisplay name - {ctx.author.display_name}\nName - {ctx.author.name}")
    ...
    @commands.group(name="матч", autohelp=False)
    ...
    async def матч_бо5(self, ctx: Context, user = None):
        ...
        try:
            ...
            return await ctx.send(f"{user} - это что?!")
        if user.id == 585141085387358258 or user.id == 159985870458322944 or user.id == 794984516443439125:
    ...
    @commands.command()
    @commands.cooldown(1, GLOBALCD, commands.BucketType.user)
    async def выпивка(self, ctx: Context):
        x=random.randint(1, 100)
        cd=await self.encooldown(ctx, spell_time=1800, spell_count=x)
        if cd:
            return await ctx.send("Вам плохо. *Рвота пьянчужки.*")
        y=random.randint(1, 5)
        self.COUNTCD[ctx.author.id][str(ctx.command)]+=y
        bottle=["эль", "пиво", "ром"]
    ...
    async def ogroquest(self, ctx: commands.GuildContext, user: discord.Member):
        ...
        gold=self.bot.get_emoji(605429832007942174)
        ...
        msg = await ctx.send(embed=embed, components=[Select(placeholder="Выбрать квест:", options=[SelectOption(label="Защита лагеря", value="Защита", emoji=dfns), SelectOption(label="Викторина", value="Викторина", emoji=vikt), SelectOption(label="Конкурс ораторов", value="Конкурс", emoji=spam), SelectOption(label="Тёмная личность", value="Казино", emoji=gold)])])
        ...
        while True:
            ...
            elif interaction.values[0] == 'Казино':
                embed = discord.Embed(title=f"Тёмная фигура возле доски объявлений подзывает к себе.", description = "\"- Нужно провернуть одно незаконное дельце в гоблинском казино, средствами я обеспечу.\"\n\nЦель: Сделать несколько ставок в казино.", colour=discord.Colour.random())
                embed.set_thumbnail(url="https://cdn.discordapp.com/emojis/606134527034916874.png")
                await msg.edit(embed=embed, components = [[Button(style = ButtonStyle.green, label = 'Взять деньги у незнакомца.'), Button(style = ButtonStyle.red, label = 'Вернуться к объявлениям.'), Button(style = ButtonStyle.blue, label = 'Пойти загорать.')]])
            else:
                return
            try:
            ...
            if responce.component.label == 'Встать на защиту лагеря!':
                ...
                for r in user.roles:
                    if r.name.startswith("Квест Защитник"):
                        await msg.edit(embed=embed, components=[])
                        return await ctx.send("Ты уже выполняешь этот квест. Не пропускай атаки огров!")
                ...
                for r in user.roles:
                    if r.name.startswith("Квест Эрудит"):
                        await msg.edit(embed=embed, components=[])
                        return await ctx.send("Ты уже выполняешь этот квест. Начать викторину можно с помощью команды `=викторина`.")
                ...
                await user.add_roles(QST)
                return await msg.edit(embed=embed, components=[])
            elif responce.component.label == 'Взять деньги у незнакомца.':
                await responce.edit_origin()
                for r in user.roles:
                    if r.name.startswith("Квест Казино"):
                        await msg.edit(embed=embed, components=[])
                        return await ctx.send("Ты уже выполняешь этот квест. Для проверки прогресса используй команду `=казино`.")
                obsch=random.randint(1000, 3000)
                QST=await ctx.guild.create_role(name='Квест Казино: '+str(obsch)+' монет', color=discord.Colour(0xA58E8E))
                embed = discord.Embed(title=f'*Тёмная фигура передаёт мешок с деньгами и листочек с инструкцией.*\n{user.display_name} начинает {QST.name}.', description = f'Цель: Неукоснительно следовать инструкции.\n\nИспользуй команду `=казино`.', colour=discord.Colour.random())
                embed.set_thumbnail(url="https://cdn.discordapp.com/emojis/605429832007942174.png")
                await user.add_roles(QST)
                return await msg.edit(embed=embed, components=[])
    ...
    @commands.command()
    async def казино(self, ctx: Context):
        author=ctx.author
        if not ctx.message.channel.name.endswith("ного_света") and not ctx.message.channel.name.endswith("казино"):
            return await ctx.send(f"*{author.display_name} подозрительно оглядывается, держа в руках мятый листок бумаги.*")
        i=0
        for r in author.roles:
            if r.name.startswith("Квест Казино"):
                i=1
        ...
        return await ctx.send(f"*В мешочке лежат {obsch} золотых монет, а инструкция гласит следующее:*\n```Дождись, пока охрана казино не будет смотреть в твою сторону и следуй этому алгоритму.\n**Далее идёт неразборчивая схема нажатия кнопок и рычагов, а внизу стоит эмблема Синдиката.**```")
    ...
    async def getmute(self, ctx, user: discord.Member, name, color):
        for role in user.roles:
            if not ctx.message.channel.permissions_for(user).send_messages and role.name==name:
                return role.name
