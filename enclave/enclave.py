...
class enclave(commands.Cog):
    ...
    async def зов_стихий(self, ctx, user: discord.Member = None):
        ...
        msg1=discord.Embed(title="Повелитель огня Рагнарос в ярости!", description="Как ты смеешь взывать ко мне?! УМРИ, НАСЕКОМОЕ!!! *У вас сгорает {x2} золотых монет, у {user2} сгорает {x3} золотых монет.*".format(x2=10-x1, user2=random.choice(ctx.message.guild.members).display_name, x3=random.randint(1, 5)), colour=discord.Colour.red())
        msg1.set_author(name="{author.display_name} делает подношение тотему огня на {x1} золотых монет.".format(author=ctx.author, x1=x1), url=author.avatar_url, icon_url=author.avatar_url)
        ...
        msg2=discord.Embed(title="Повелитель огня Рагнарос доволен!".format(author=ctx.author, x1=x1), description="Восстань, слуга пламени! Поглоти их плоть! *Повелитель огня назначает вас своим мажордомом.*", colour=discord.Colour.dark_red())
        msg2.set_author(name="{author.display_name} делает подношение тотему огня на {x1} золотых монет.".format(author=ctx.author, x1=x1), url=author.avatar_url, icon_url=author.avatar_url)
        ...
        msg3=discord.Embed(title="Повелитель огня Пеплорон обращает на вас свой взор!".format(author=ctx.author, x1=x1), description="На колени, смертный! *Пеплорон сжигает {x1} золотых монет у {user.display_name}.*".format(x1=x1, user=user), colour=discord.Colour.orange())
        msg3.set_author(name="{author.display_name} делает подношение тотему огня на {x1} золотых монет.".format(author=ctx.author, x1=x1), url=author.avatar_url, icon_url=author.avatar_url)
        msg3.set_thumbnail(url="https://wow.zamimg.com/uploads/screenshots/normal/660184-.jpg")
        msg4=discord.Embed(title="Герцог Гидраксис в ярости!".format(author=ctx.author, x1=x1), description="Как ты смеешь взывать ко мне?! Умри, насекомое!!!", colour=discord.Colour.blue())
        msg4.set_author(name="{author.display_name} делает подношение тотему воды на {x1} золотых монет.".format(author=ctx.author, x1=x1), url=author.avatar_url, icon_url=author.avatar_url)
        msg4.set_thumbnail(url="https://wow.zamimg.com/modelviewer/live/webthumbs/npc/246/58870.png")
        msg5=discord.Embed(title="Повелитель воды Нептулон в ярости!".format(author=ctx.author, x1=x1), description="Нерадивое сухопутное! Ты решил потревожить владыку вод?! Почувствуй силу чистой воды!!!", colour=discord.Colour.dark_blue())
        msg5.set_author(name="{author.display_name} делает подношение тотему воды на {x1} золотых монет.".format(author=ctx.author, x1=x1), url=author.avatar_url, icon_url=author.avatar_url)
        msg5.set_thumbnail(url="https://wow.blizzwiki.ru/images/thumb/9/95/Neptulon.jpg/200px-Neptulon.jpg")
        msg6=discord.Embed(title="Повелитель воздуха Громораан в ярости!".format(author=ctx.author, x1=x1), description="Я дарую тебе силу ветров!", colour=0xD0D0D0)
        msg6.set_author(name="{author.display_name} делает подношение тотему воздуха на {x1} золотых монет.".format(author=ctx.author, x1=x1), url=author.avatar_url, icon_url=author.avatar_url)
        msg6.set_thumbnail(url="https://wow.zamimg.com/uploads/screenshots/small/683871.jpg")
        msg7=discord.Embed(title="Повелитель воздуха Алакир в ярости!".format(author=ctx.author, x1=x1), description="Жалкий смертный, твои попытки приводят меня в ЯРОСТЬ!!!", colour=0x808080)
        msg7.set_author(name="{author.display_name} делает подношение тотему воздуха на {x1} золотых монет.".format(author=ctx.author, x1=x1), url=author.avatar_url, icon_url=author.avatar_url)
        ...
        msg8=discord.Embed(title="Теразан в ярости!".format(author=ctx.author, x1=x1), description="Смертные погубили моё дитя! Почувствуй же мой гнев!", colour=discord.Colour.gold())
        msg8.set_author(name="{author.display_name} делает подношение тотему земли на {x1} золотых монет.".format(author=ctx.author, x1=x1), url=author.avatar_url, icon_url=author.avatar_url)
        msg8.set_thumbnail(url="https://rpwiki.ru/images/thumb/d/d6/Теразан.jpg/250px-Теразан.jpg")
        msg9=discord.Embed(title="Теразан удовлетворена!".format(author=ctx.author, x1=x1), description="Ты решил потревожить мать-скалу? Получи моё благославение.", colour=discord.Colour.dark_gold())
        msg9.set_author(name="{author.display_name} делает подношение тотему земли на {x1} золотых монет.".format(author=ctx.author, x1=x1), url=author.avatar_url, icon_url=author.avatar_url)
        ...
        await ctx.send (embed=random.choice(mass))
