...
class UserProfile:
    ...
    async def _downgrade_level(self, member):
        ...
        pastlvl = await self.givexp(lvl-1)
        xp = await self.data.member(member).exp()
        while xp < pastlvl and lvl > 0:
            lvl -= 1
            pastlvl = await self.givexp(lvl-1)
    ...
    async def _check_exp(self, member):
        lvl = await self.data.member(member).level()
        lvlup = await self.givexp(lvl)
        lvldown = await self.givexp(lvl-1)
        xp = await self.data.member(member).exp()
        if xp >= lvlup:
    ...
    async def _check_role_member(self, member):
        ...
        if lvl%5==0 and lvl!=0:
            rl=discord.utils.get(member.guild.roles, id=696014224442392717)
